...
async function run() {
  ...
  await WorkspaceMembership.create({ workspaceId, userId: new mongoose.Types.ObjectId(reviewerRes.body.user.id), role: "Reviewer" });
  await WorkspaceMembership.create({ workspaceId, userId: new mongoose.Types.ObjectId(viewerRes.body.user.id), role: "Viewer" });
  ...
  const adminTokenRes = await req("/auth/login", "POST", { email: "admin" + rand + "@test.com", password: "password123", selectedRole: "Admin" });
  const adminToken = adminTokenRes.body.token;

  const reviewerTokenRes = await req("/auth/login", "POST", { email: "rev" + rand + "@test.com", password: "password123", selectedRole: "Reviewer" });
  const reviewerToken = reviewerTokenRes.body.token;

  const viewerTokenRes = await req("/auth/login", "POST", { email: "view" + rand + "@test.com", password: "password123", selectedRole: "Viewer" });
  const viewerToken = viewerTokenRes.body.token;
